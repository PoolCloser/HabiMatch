...
client = TestClient(app, raise_server_exceptions=False)  # ← allows 500s to be caught as responses
...
def mock_current_user():
    return {"sub": TEST_USER_ID}  # ← was {"id": ...}
...
def reset_overrides():
    app.dependency_overrides[require_auth] = mock_current_user  # ← was get_current_user
    yield
...
class TestCreateLifestyle:
    ...
    def test_create_lifestyle_no_auth(self):
        ...
        assert response.status_code == 401  # ← was 422

    @pytest.mark.parametrize("field", [
        "sleep_time_score", "sleep_tolerance_score", "clean_behavior_score",
        "clean_tolerance_score", "noise_behavior_score", "noise_tolerance_score",
        "guest_behavior_score", "guest_tolerance_score", "conflict_style",
    ])
    ...
    @pytest.mark.parametrize("field", [
        "smokes", "ok_with_smoking", "has_pets", "ok_with_pets", "study_or_wfh"
    ])
...
class TestUpdateLifestyle:
    ...
    def test_update_lifestyle_invalid_score(self):
        response = client.put("/lifestyle/", json={"sleep_time_score": 0}, headers={"Authorization": "Bearer faketoken"})
        assert response.status_code == 422
